...
public class HuntingIndustry extends Industry {
	...
	public static void load() {
		GoodFactory.create("rabbit", 1, 0);
		ptr = new HuntingIndustry();
		Industry.put("hunting", ptr);
	}

	public static HuntingIndustry get() {
		return ptr;
	}

	protected HuntingIndustry() {
		createThis = Good.get("meat");
		rabbit = Good.get("rabbit");
	...
	public long produce(ProvinceIndustry indus, Collection<Pop> pops, int durationInDay, long wish) {
		...
		long nbRabbit = (indus.getStock().getLong(rabbit)); // 100kg per Rabbit

		// temp booststrap (there are always at least two rabbit)
		if (nbRabbit == 0) {
			indus.getStock().put(rabbit, nbForest / 2);
			nbRabbit = nbForest / 2;
		}
		...
		if (newRabbit <= 0) {
			if (GlobalRandom.aleat.getInt(1000,
					(int) (prv.pourcentPrairie * 1000000)) < (int) (nbRabbit * durationInDay * 1.39)) {
				newRabbit = 1;
			}
		}
		...
		nbRabbitToSell = (long) (durationInDay * 6 * nbMens * (nbRabbit / (double) nbForest));
		nbRabbit -= nbRabbitToSell;

		logln(",\"nbRabbitToSell\":" + nbRabbitToSell + ",\"calculus:\":\"nbDay(" + durationInDay + ")*6*" + nbMens
				+ (nbRabbit / (float) nbForest) + "\"");
		// note: over-hunt can happen easily (no hunter limit), it's intended.

		// set new livestock
		indus.getStock().put(rabbit, nbRabbit);
	...
	public long getMenWish(ProvinceIndustry indus, double currentConsumptionPD) {
		...
		long nbForest = (int) ((prv.pourcentForet * prv.surface * 100)); // 1 hectare per Rabbit
		long nbRabbit = (indus.getStock().getLong(rabbit)); // 100kg per Rabbit
		long nbMensWish = (long) (currentConsumptionPD / (6 * (nbRabbit / (double) nbForest)));
		return nbMensWish;
	}
