...
public class HuntingSportIndustry extends Industry {
	...
	public static void load() {
		GoodFactory.create("cerf", 1, 0);
		ptr = new HuntingSportIndustry();
		Industry.put("bighunting", ptr);
	}

	public static HuntingSportIndustry get() {
		return ptr;
	}
	...
	public long produce(ProvinceIndustry indus, Collection<Pop> pops, int durationInDay, long wish) {
		...
		long nbCerf = (indus.getStock().getLong(cerf)); // 100kg per Rabbit

		// temp booststrap (there are always at least two rabbit)
		if (nbCerf == 0) {
			indus.getStock().put(cerf, nbForest / 2);
			nbCerf = nbForest / 2;
		}
		...
		if (newCerf <= 0) {
			if (GlobalRandom.aleat.getInt(1000,
					(int) (prv.pourcentPrairie * 1000000)) < (int) (nbCerf * durationInDay * 1.39)) {
				newCerf = 1;
			}
		}
		...
		logln(",\"nbCerfToSell\":" + nbRabbitToSell + ",\"calculus:\":\"nbDay(" + durationInDay + ")*" + nbMens
				+ (nbCerf / (float) nbForest) + "\"");

		// note: over-hunt can happen easily (no hunter limit), it's intended.

		// set new livestock
		indus.getStock().put(cerf, nbCerf);
	...
	public long getMenWish(ProvinceIndustry indus, double currentConsumptionPD) {
		...
		long nbMensWish = (long) (currentConsumptionPD / (nbCerf / (double) nbForest));
		return nbMensWish;
	}
